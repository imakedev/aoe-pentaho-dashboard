	...
	jQuery(document).ready(function($) {
		...
		//	$("#tabs").tabs();
			...
			$('a[rel*=facebox]').facebox({
				loadingImage : ''+_path+'/chartLib/images/loading.gif',
				closeImage   : ''+_path+'/chartLib/images/closelabel.png'
			});
			"WebContent/Dashboard/Finance_bk.jsp"*/
			 initChart();
			
			 zingchart.node_click = function(node){
				// if(node["graphid"].indexOf("pileId")!=-1)
				// alert(node["graphid"]+" value="+node["plotindex"])
				//var xx= zingchart.exec("pileId", "getdata");
				//alert("xx="+xx)
				var text_node=node["text"];
				var text_arr=text_node.split(": ");
				if(text_arr[1]!='0'){
					renderBarOfChart(text_arr[0]);
				}
				 
			     /*  alert("Node Clicked - Key: " + node["key"] + 
			       " Value: " + node["value"] + "  index = " + node["plotindex"] + " GraphID = " + node["graphid"]);  */
			}
		});
	/* //var data = '{ "graphset" : [ { "plot" : {},"scale-x" : {"values": ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep"]},"scale-y" : {},"type" : "line","legend" : {"position" : "100% 0%","margin-top" : 10,"margin-right" : 10,"margin-left" : 10,"margin-bottom" : 10,"fixed" : true,"minimize" : false},"title" : {"text" : "Loaded from API","position" : "0% 0%","margin-top" : 10,"margin-right" : 0,"margin-left" : 0,"margin-bottom" : 10},"series" : [ {"values" : [57,46,74,30,29,89,8,28,54], "text" : "Item 0"},{"values" : [29,88,16,78,46,10,13,0,67], "text" : "Item 1"} ] } ] }';
		zingchart.exec("zingchart-graph", "setdata", '{"data":' + data + '}');  */
	//	Home  +  Download  +  Purchase  +  Learn  +  Support  +  Support Forum  +  About  +  Privacy  +  Terms  +  Site Map
...
((ค่า Period ปัจจุบัน – ค่า Period ก่อนหน้า)/ค่า Period ก่อนหน้า)*100 */
 					  ...
 					  /*  var  actual=((thisActual-lastActual)/lastActual)*100
 					  var  budget=((thisBudget-lastBudget)/lastBudget)*100 */
...
สูตรการคิด PieChartRatio คือ ThisPeriodActual ของแต่ละธุรกิจ/ ThisPeriodActual ของทุกธุรกิจ เช่น ratio ของ restaurant คือ 110/690 */
					...
					callback:function(data){	 
        				...
        				var percentVarianceMONTHLY="";  
				 		   ...
				 		   var tabParam=orgType!=null?("&tab="+orgType):"";
				    		   $("#"+graph_id+"_legend").html("<a onclick=showPopup(\""+_path+"/Dashboard/FinancePopup_new.jsp?topic="+gTypeG[graphIndex]+"&sb="+sumby+tabParam+"\") rel=\"facebox\">"+
				     				"<img src=\""+_path+"//Dashboard/images/Popup.jpg\" border=\"0\"/> <font color=\"blue\" size=\"1\">Drill Down</font>"+
		     			    "</a>");   
						    }
     ...
     function showPopup(div){
    	...
    	var str="<table width=\"700\" height=\"300\" border=\"1\" align=\"center\" style=\"text-decoration:none;\">"+
    	 "<tr><th align=\"center\"><font size=\"3\">%ต้นทุนและค่าใช้จ่ายเทียบยอดขาย</font></th></tr>"+
    	 "<tr><td align=\"center\" id=\"gxx1\"></td></tr>"+
    	 "</table>";
    	 ...
    	 //jQuery.facebox({ div:div});
    	// jQuery.facebox({ div:'#gxx1'});
     }
/*
		jQuery(document).ready(function($) {
			$('a[rel*=facebox]').facebox({
...
<td>
  ...
  <tr>
	  ...
	  <td bgcolor="#CD853F"><font size="3" color="#FFFFFF"><div align=center id="sales_growth">%Sales Growth sales</div></font></td>
  </tr>
